...
3) Class now supports file attachments
  ...
  with the buffer used for encoding base64 attachments
	...
	GetLastError
	5) A number of extra ATLASSERTs have been added
	...
	2) Fixed a major bug which was causing an ATLASSERT when the CSMTPAttachment
...
{
	ATLASSERT(sFilename.GetLength());  //Empty Filename !
	...
	{
		ATLTRACE(_T("Failed to get the status for file %s, probably does not exist\n"), sFilename);
		return FALSE;
	}
	...
	{
		ATLTRACE(_T("Failed to open file to be attached\n"));
		return FALSE;
	}
	
	//read in the contents of the input file
	DWORD m_size = infile.GetLength();
	char* pszIn = new char[m_size];
	infile.Read(pszIn, m_size);
	
	//allocate the encoded buffer
	int nOutSize = Base64BufferSize(m_size);
	m_pszEncoded = new char[nOutSize];
	
	//Do the encoding
	EncodeBase64(pszIn, m_size, m_pszEncoded, nOutSize, &m_nEncodedSize);
...
BOOL CSMTPAttachment::EncodeBase64(const char* pszIn, int nInLen, char* pszOut, int nOutSize, int* nOutLen)
{
	...
	ATLASSERT(pszOut);
	ATLASSERT(nOutSize);
	ATLASSERT(nOutSize >= Base64BufferSize(nInLen));
...
{
	ATLASSERT(pAttachment->GetFilename().GetLength()); //an empty filename !
...
CString CSMTPMessage::GetHeader() const
{
	...
	struct tm* ptmTemp = localtime(&tmnow);
	if (ptmTemp == NULL ||
		!_tcsftime(szBuffer, _countof(szBuffer), _T("%a, %d %b %Y %H:%M:%S %Z"), ptmTemp))
		szBuffer[0] = '\0';
	CString sDate = szBuffer;
...
{
	ATLASSERT(sRecipients.GetLength()); //An empty string is now allowed
		...
		if ((buf[pos] == _T(',')) || (buf[pos] == _T(';')) || (buf[pos] == 0))
		{
			...
			if (nMark >= 0)
			{
				...
				if (nMark2 < nMark)
				{
					...
					ATLTRACE(_T("An error occurred while parsing the recipients string\n"));
					return FALSE;
				}
...
CSMTPConnection::CSMTPConnection()
{
	m_bConnected = FALSE;
#ifdef _DEBUG
	m_dwTimeout = 40000; //default timeout of 20 seconds when debugging
#else
	m_dwTimeout = 40000;  //default timeout of 2 seconds for normal release code
#endif
}
...
BOOL CSMTPConnection::Connect(LPCTSTR pszHostName, int nPort)
{
	...
	{
		ATLTRACE(_T("Could not connect to the SMTP server %s on port %d, GetLastError returns: %d\n"), pszHostName, nPort, WSAGetLastError());
		return FALSE;
	}
	else
	{
		...
		{
			ATLTRACE(_T("An unexpected SMTP login response was received\n"));
			Disconnect();
			return FALSE;
		}
		...
		{
			Disconnect();
			ATLTRACE(_T("An unexpected error occurred while sending the HELO command\n"));
			return FALSE;
		}
		...
		{
			Disconnect();
			ATLTRACE(_T("An unexpected HELO response was received\n"));
			return FALSE;
		} 
...
BOOL CSMTPConnection::Disconnect()
{
	...
	if (m_bConnected)
	{
		...
		if (!m_SMTP.Send(sBuf, nCmdLength))
			ATLTRACE(_T("Failed in call to send QUIT command, GetLastError returns: %d\n"), GetLastError());
		...
		if (!bSuccess)
		{
			...
			ATLTRACE(_T("An unexpected QUIT response was received\n"));
		}
		...
		ATLTRACE(_T("Already disconnected from SMTP server, doing nothing\n"));
...
BOOL CSMTPConnection::SendMessage(CSMTPMessage& Message)
{
	...
	ATLASSERT(Message.m_From.m_sEmailAddress.GetLength());
	CString sBuf;
	...
	{
		ATLTRACE(_T("Failed in call to send MAIL command, GetLastError returns: %d\n"), GetLastError());
		return FALSE;
	}
	...
	{
		SetLastError(ERROR_BAD_COMMAND);
		ATLTRACE(_T("An unexpected MAIL response was received\n"));
		return FALSE;
	} 
	...
	ATLASSERT(Message.GetNumberOfRecipients(CSMTPMessage::TO) + 
	...
	{
		SetLastError(ERROR_BAD_COMMAND);
		ATLTRACE(_T("An unexpected DATA response was received\n"));
		return FALSE;
	} 
	...
	{
		ATLTRACE(_T("Failed in call to send the header, GetLastError returns: %d\n"), GetLastError());
		return FALSE;
	}
	...
	if (Message.m_Attachments.GetSize())
	{
		...
		{
			ATLTRACE(_T("Failed in call to send the body header, GetLastError returns: %d\n"), GetLastError());
			return FALSE;
		}
	}
	...
	for (i=0; i<Message.m_Attachments.GetSize(); i++)
	{
		...
		{
			ATLTRACE(_T("Failed in call to send Mime attachment header, GetLastError returns: %d\n"), GetLastError());
			return FALSE;
		}
		...
		{
			ATLTRACE(_T("Failed in call to send the attachment, GetLastError returns: %d\n"), GetLastError());
			return FALSE;
		}
	}
	...
	{
		ATLTRACE(_T("Failed in call to send end of message indicator, GetLastError returns: %d\n"), GetLastError());
		return FALSE;
	}
	...
	{
		SetLastError(ERROR_BAD_COMMAND);
		ATLTRACE(_T("An unexpected end of message response was received\n"));
		return FALSE;
	} 
...
BOOL CSMTPConnection::SendRCPTForRecipient(CSMTPAddress& recipient)
{
	...
	{
		ATLTRACE(_T("Failed in call to send RCPT command, GetLastError returns: %d\n"), GetLastError());
		return FALSE;
	}
	...
	{
		SetLastError(ERROR_BAD_COMMAND);
		ATLTRACE(_T("An unexpected RCPT response was received\n"));
		return FALSE;
	} 
